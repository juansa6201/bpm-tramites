...
import { AsignarTramiteInput } from '../../dto/tramites/asignar-tramite.input';
...
const ACCION = AccionMovimiento.ASIGNAR;

/**
 * Acción del INTERNO (supervisor/admin): reasigna el trámite a otro interno.
 * Efecto colateral: cambia el usuario asignado. No cambia de estado.
 */
export class AsignarTramiteUseCase {
  ...
  async execute(input: AsignarTramiteInput): Promise<TramiteTransicionResult> {
    return this.uow.runInTransaction(async (repos) => {
      ...
      tramite.asignarA(input.usuarioAsignadoId);
